# Meeple — fiche trait

> Fiche GÉNÉRÉE depuis CommunityDragon patch 17.4 — **NE PAS ÉDITER À LA MAIN**.
> Régénérer : `.venv/bin/python -m scripts.generate_fiches`

- **apiName** : `TFT17_Astronaut`
- **Paliers d'activation** : 3 / 5 / 7 / 10

## Paliers

| Min unités | Max | Style | Variables |
|---|---|---|---|
| 3 | 4 | 1 | `BonusHealth`=100, `Meeps`=2 |
| 5 | 6 | 3 | `BonusHealth`=400, `Meeps`=3 |
| 7 | 9 | 5 | `BonusHealth`=400, `Meeps`=4 |
| 10 | 25000 | 6 | `BonusHealth`=500, `Meeps`=6 |

## Statut moteur

| Variable | Application moteur |
|---|---|
| `BonusHealth` | ✅ stat auto-appliquée (hp) |
| `Meeps` | 🟡 non appliquée automatiquement (effet codé uniquement si présent dans le long-tail moteur) |

## Champions avec ce trait

- [Bard](../champions/TFT17_Bard.md)
- [Corki](../champions/TFT17_Corki.md)
- [Fizz](../champions/TFT17_Fizz.md)
- [Gnar](../champions/TFT17_Gnar.md)
- [Meepsie](../champions/TFT17_IvernMinion.md)
- [Poppy](../champions/TFT17_Poppy.md)
- [Rammus](../champions/TFT17_Rammus.md)
- [Veigar](../champions/TFT17_Veigar.md)

