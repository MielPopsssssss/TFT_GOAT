# Primordian — fiche trait

> Fiche GÉNÉRÉE depuis CommunityDragon patch 17.4 — **NE PAS ÉDITER À LA MAIN**.
> Régénérer : `.venv/bin/python -m scripts.generate_fiches`

- **apiName** : `TFT17_Primordian`
- **Paliers d'activation** : 2 / 3

## Paliers

| Min unités | Max | Style | Variables |
|---|---|---|---|
| 2 | 2 | 1 | `DamageMultiplier`=1, `DamageTakenPercentModifier`=0.08, `PercentMoreSwarmlings`=45, `{3ed71f24}`=6 |
| 3 | 25000 | 5 | `DamageMultiplier`=1.45, `DamageTakenPercentModifier`=0.08, `PercentMoreSwarmlings`=45, `{3ed71f24}`=6 |

## Statut moteur

| Variable | Application moteur |
|---|---|
| `DamageMultiplier` | 🟡 non-stat — effet appliqué uniquement si codé (long-tail) |
| `DamageTakenPercentModifier` | 🟡 non-stat — effet appliqué uniquement si codé (long-tail) |
| `PercentMoreSwarmlings` | 🟡 non-stat — effet appliqué uniquement si codé (long-tail) |
| `{3ed71f24}` | 🟡 non-stat — effet appliqué uniquement si codé (long-tail) |

## Champions avec ce trait

- [Bel'Veth](../champions/TFT17_Belveth.md)
- [Briar](../champions/TFT17_Briar.md)
- [Rek'Sai](../champions/TFT17_RekSai.md)

