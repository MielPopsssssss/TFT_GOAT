...
# Passive Party Crasher : 1 bolt / @BoltCooldown@s (2s). Cycle de cast ~6s (20/100 mana,
# AS 0.9) -> ~3 bolts amortis par cast (pas de hook périodique champion dans le moteur).
_BLITZ_BOLTS_PER_CAST = 3


def _blitzcrank(caster, allies, enemies, ctx) -> None:
    # Party Crasher: disco ball -> uppercut + explosion AoE + KNOCKUP du clump
    # (GrooveDurationPerTarget par ennemi touché) ; passive bolt amortie dans le cast.
    target = ctx.highest_hp_enemy(caster) or ctx.target_of(caster)
    if target is None:
        return
    ap = caster.ap / 100.0
    ctx.deal_magic(caster, target, caster.ability_value("UppercutDamage") * ap)
    groove = caster.ability_value("GrooveDurationPerTarget", 1.0)
    ctx.stun(target, groove)  # knockup de la cible
    explosion = caster.ability_value("ExplosionDamage") * ap
    for e in ctx.enemies_around(caster, target, 1):  # approx: disco-ball crash AoE around target
        ctx.deal_magic(caster, e, explosion)
        if e is not target:
            ctx.stun(e, groove)  # knockup du clump autour de la cible
    bolt = caster.ability_value("BoltDamage") * ap
    for _ in range(_BLITZ_BOLTS_PER_CAST):  # passive amortie : bolt sur le plus gros HP
        ctx.deal_magic(caster, target, bolt)
...
def _nunu(caster, allies, enemies, ctx) -> None:
    ...
    for e in ctx.enemies_around(caster, center, 2):
